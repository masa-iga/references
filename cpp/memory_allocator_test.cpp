...
namespace { // for constants
    const size_t kMemorySize = 1 * 1000 * 1000 * 1000;
...
namespace { // for functions
    ...
    TEST_F(AllocatorTest, sizeAlignCurrent)
    {
        VPRINTF("base address 0x%p\n", base_);

        {
            const size_t size = 0x100;
            const size_t align = 0x4;
            void* const alloc_addr = allocator_->alloc(size, align);

            const size_t alloc_size = reinterpret_cast<size_t>(alloc_addr) - reinterpret_cast<size_t>(base_);
        ...
        VPRINTF("current 0x%p\n", allocator_->getMarker());
